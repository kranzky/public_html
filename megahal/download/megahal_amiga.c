/*
	...
	that is the conditions set forth by the GNU General Public License.
	...
	the original algorithm was changed. Contact: Vincentpojken@altavista.net
*/
...
#include <stdlib.h>
#include <stdarg.h>
#if !defined(AMIGA_OLD) && !defined(AMIGA_V38) && !defined(__mac_os)
#include <malloc.h>
#endif
#include <string.h>
#include <signal.h>
...
#include <proto/dos.h>
#endif
#if defined(AMIGA_OLD)
#include <ctype.h>
#endif
#if defined(AMIGA_V38)
...
#if defined(AMIGA_OLD) || defined(AMIGA_V38)
	char __stdiowin[] = "CON:15/56/640/200/MegaHAL version 8 revision 7";
	char __stdiov37[] = "";
#endif
#ifdef AMIGA_V38
...
#if defined(AMIGA_OLD) || defined(AMIGA_V38)
	"$VER: MegaHAL Version 8 revision 6 (1999.09.27)";
#endif
#ifdef AMIGA_V38
...
"|         For license conditions, see separate file 'License.txt'        |\n"
"+------------------------------------------------------------------------+\n"
);
...
char *read_input(char *prompt)
{
	...
	while(isspace((unsigned long) input[length-1])) --length;
	input[length]='\0';
...
void capitalize(char *string)
{
	...
	for(i=0; i<(int)strlen(string); ++i) {
		if(isalpha((unsigned long) string[i])) {
			if(start==TRUE) string[i]=(char)toupper((unsigned long) string[i]);
			else string[i]=(char)tolower(string[i]);
			start=FALSE;
		}
		if((i>2)&&(strchr("!.?", string[i-1])!=NULL)&&(isspace((unsigned long) string[i])))
			start=TRUE;
	}
}
...
void upper(char *string)
{
	...
	for(i=0; i<(int)strlen(string); ++i) string[i]=(char)toupper((unsigned long) string[i]);
}
...
char *format_output(char *output)
{
   ...
   for(i=0; i<(int)strlen(output); ++i) {
      if((l==0)&&(isspace((unsigned long) output[i]))) continue;
...
BYTE2 add_word(DICTIONARY *dictionary, STRING word)
{
	...
	if(dictionary->index==NULL) {
		dictionary->index=(BYTE2 *)malloc((size_t) (sizeof(BYTE2)*
		(dictionary->size)));
	} else {
		dictionary->index=(BYTE2 *)realloc((BYTE2 *)
		(dictionary->index),(size_t) (sizeof(BYTE2)*(dictionary->size)));
	}
	if(dictionary->index==NULL) {
	...
	if(dictionary->entry==NULL) {
		dictionary->entry=(STRING *)malloc((size_t) (sizeof(STRING)*(dictionary->size)));
	} else {
		dictionary->entry=(STRING *)realloc((STRING *)(dictionary->entry),
		(size_t) (sizeof(STRING)*(dictionary->size)));
	}
	if(dictionary->entry==NULL) {
...
void save_dictionary(FILE *file, DICTIONARY *dictionary)
{
	...
	for(i=0; i<dictionary->size; ++i) {
		...
		progress(NULL, i, (int) dictionary->size);
	}
	progress(NULL, 1, 1);
}
...
void train(MODEL *model, char *filename)
{
   ...
   rewind(file);
	...
	while(!feof(file)) {
		...
		progress(NULL, (int) ftell(file), length);
...
void make_words(char *input, DICTIONARY *words)
{
	...
	while(1) {
		...
		if(boundary(input, offset)) {
			 ...
			 */
			if(words->entry==NULL)
				words->entry=(STRING *)malloc((size_t) (words->size+1)*sizeof(STRING));
			else
				words->entry=(STRING *)realloc(words->entry, (size_t) (words->size+1)*sizeof(STRING));
...
void make_greeting(DICTIONARY *words)
{
	...
	free_dictionary(words);
	if(grt->size>0) (void)add_word(words, grt->entry[rnd((int)grt->size)]);
}
...
char *generate_reply(MODEL *model, DICTIONARY *words)
{
	...
	do {
		...
		progress(NULL, (int) (time(NULL)-basetime),timeout);
...
DICTIONARY *reply(MODEL *model, DICTIONARY *keys)
{
	...
	while(TRUE) {
		...
		if(replies->entry==NULL)
			replies->entry=(STRING *)malloc((size_t) (replies->size+1)*sizeof(STRING));
		else
			replies->entry=(STRING *)realloc(replies->entry, (size_t) (replies->size+1)*sizeof(STRING));
		if(replies->entry==NULL) {
...
float evaluate_reply(MODEL *model, DICTIONARY *keys, DICTIONARY *words)
{
	...
	if(num>=8) entropy/=(float)sqrt((double) num-1);
...
int rnd(int range)
{
	...
	return(rand()%range);
#else
	return(int)(floor(drand48()*(double)(range)));
#endif
}
