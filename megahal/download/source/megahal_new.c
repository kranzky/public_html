...
void initialize_list(char *, DICTIONARY);
#ifdef __mac_os
bool initialize_speech(void);
#endif
bool initialize_status(char *);
void initialize_swap(char *, SWAP);
...
COMMAND command[] = {
	{ { 5, 0 }, "change to another MegaHAL personality", BRAIN },
	{ { 5, 5 }, "toggles MegaHAL's typing delay (off by default)", DELAY },
	{ { 4, 10 }, "exits the program without saving MegaHAL's brain", EXIT },
	{ { 4, 14 }, "displays this message", HELP },
	{ { 4, 18 }, "toggle the progress display (on by default)", PROGRESS },
	{ { 4, 22 }, "quits the program and saves MegaHAL's brain", QUIT },
	{ { 4, 26 }, "saves the current MegaHAL brain", SAVE },
	{ { 6, 30 }, "toggles MegaHAL's speech (off by default)", SPEECH },
	{ { 5, 36 }, "changes MegaHAL's thinking time (2 by default)", THINK },
	{ { 5, 41 }, "switches to voice specified", VOICE },
	{ { 6, 46 }, "list available voices for speech", VOICELIST }
};
...
int main(int argc, char **argv)
{
	char *input=NULL;
	char *output=NULL;
	DICTIONARY words={0, NULL, NULL};
	DICTIONARY greets={0, NULL, NULL};
	MODEL model;
	int position=0;
...
{
	dictionary_buffer=
		strdup("BRAINDELAYEXITHELPPROGQUITSAVESPEECHTHINKVOICEVOICES<ERROR><FIN>");
}
...
BYTE2 add_word(DICTIONARY dictionary, STRING word)
{
	...
	dictionary.entry[dictionary.size-1].index=word.index;
...
int wordcmp(STRING word1, STRING word2)
{
	...
	for(i=0; i<bound; ++i)
		if(toupper(dictionary_buffer[word1.index+i])!=
			toupper(dictionary_buffer[word2.index+i]))
			return((int)(toupper(dictionary_buffer[word1.index+i])-
				toupper(dictionary_buffer[word2.index+i])));
 ...
 */
void free_dictionary(DICTIONARY words)
{
	words.size=0;
	if(words.index!=NULL) free(words.index);
	if(words.entry!=NULL) free(words.entry);
	words.index=NULL;
	words.entry=NULL;
}
 ...
 */
void new_model(MODEL *model, int order)
{
	STRING word={ 7, 52 };
	STRING end={ 5, 59 };
	...
	(void)add_word(model.dictionary, word);
	(void)add_word(model.dictionary, end);
}
...
void add_swap(SWAP list, char *s, char *d)
{
	...
	list.from[list.size-1].index=add_string(s);
	list.to[list.size-1].length=strlen(d);
	list.to[list.size-1].index=add_string(d);
}
 ...
 */
void initialize_swap(char *filename, SWAP list)
{
	FILE *file=NULL;
	char buffer[1024];
	char *from;
...
{
	swap.size=0;
	if(swap.from!=NULL) free(swap.from);
	if(swap.to!=NULL) free(swap.to);
	swap.from=NULL;
	swap.to=NULL;
}
 ...
 */
void initialize_list(char *filename, DICTIONARY list)
{
	FILE *file=NULL;
	STRING word;
	char *string;
...
void remove_string(STRING word)
{
	if(strlen(dictionary_buffer)-word.index!=word.length)
		error("remove_string", "Not on the end of buffer");
	dictionary_buffer=realloc(dictionary_buffer, word.index+1);
	dictionary_buffer[word.index]='\0';
}
...
int add_string(char *string)
{
	...
	index=strlen(dictionary_buffer);
	dictionary_buffer=realloc(dictionary_buffer, index+strlen(string)+1);
	strcat(dictionary_buffer, string);
	return(index);
}
